...
public class UserInfoController {
	...
	public String GoPersonalCenter(HttpServletRequest request,
		...
		if (use == null) {
			response
					.sendRedirect("https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx38b32f527759debd&redirect_uri=http%3A%2F%2Fxiguashu.jiriweb.com%2FgetUser.do&response_type=code&scope=snsapi_userinfo&state=STATE#wechat_redirect");
		} else {
	...
	public String signin(HttpServletRequest request,
		...
		// 查询签到时间
		List<ScoreRecords> src = scorerecords.findScoreRecordsBytitle("签到积分",
				id);
	...
	public String myspred(HttpServletRequest request,
	 ...
	 */
	/*
	 * @RequestMapping("cpQRCode.do") public void cpQRCode(HttpServletRequest
	 * request,HttpServletResponse response) throws Exception{ QRCode c = new
	 * QRCode(); c.test1(response,"",""); }
	 */
	...
	public String getUser(HttpServletRequest request,
		...
		if (!"authdeny".equals(code)) {
			// 获取网页授权access_token
			WeixinOauth2 weixinOauth2 = w.getOauth2AccessToken(
					"wx777f0b5b69efafc7", "dfe6cb97267098d77713122261ef0f32",
					code);
			// 网页授权接口访问凭证
			...
			if (us == null) {
				Users use = new Users();
				use.setAddress(snsUserInfo.getCountry() + ","
						+ snsUserInfo.getCity());
	...
	public String Gomyproduct(HttpServletRequest request) {
		...
		JSONObject jsonObject = WeixinUtil
				.httpRequest(getTokenUrl, "GET", null);
		...
		String timestamp = Long.toString(System.currentTimeMillis() / 1000);// shi
																			// jian
																			// chui
		...
		List<Orderserviceproduct> list = orderproduct
				.findybOrderProductsbycusid(us.getId());
	...
	// public String Gomyproductyanbao(HttpServletRequest
	// request,HttpServletResponse response,String SerialNumber){
	...
	public String personinfoset(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		HttpSession session = request.getSession();
		String openID = (String) session.getAttribute("openID");
		Users use = userservice.findUsersByoppId(openID);
	...
	public String zhifu(HttpServletRequest request, HttpServletResponse response)
		...
		System.out.println("pay.jsp?appid=" + appid2 + "&timeStamp="
				...
				+ "&sign=" + finalsign);
		/*
		 * response.sendRedirect("pay.jsp?appid="+appid2+"&timeStamp="+timestamp+
		 * "&nonceStr="+nonceStr2+"&package="+packages+"&sign="+finalsign);
		 */
		return "pay.jsp?appid=" + appid2 + "&timeStamp=" + timestamp
	...
	public String zhifuhuidiao(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
